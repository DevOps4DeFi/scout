...
def main():
    ...
    ### LP Tokens
        ...
        for token, address in treasury_tokens.items():
            console.print(f'Processing wallet balances for [bold]{token}:{tokenAddress}...')
            ...
            for metric in info.describe():
                ...
                try:
                    wallets_gauge.labels(metric["walletName"], metric["walletAddress"], "BNB", "None", "usdBalance").set(float(w3.fromWei(w3.eth.getBalance(wallet_web3_addr), 'ether')) * usd_prices_by_token_address[treasury_tokens["WBNB"]])

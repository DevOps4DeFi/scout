...
def main():
    sett_gauge = Gauge(
        "sett", "Data from Badger Vaults", ["sett", "param", "tokenAddress", "token"]
    )
    ...
    lpTokens_gauge = Gauge(
        "lptokens", "LP Token data", ["token", "param", "tokenAddress"]
    )
    crvtoken_gauge = Gauge(
        "crvtokens", "CRV token data", ["token", "param", "tokenAddress"]
    )
    wallets_gauge = Gauge(
        "wallets",
    ...
    for block in chain.new_blocks(height_buffer=1):
        ...
        console.rule(
            title=f"[green]{block.number} at {timestamp} step number {step} and block numbne {block.number}"
        )
        block_gauge.set(block.number)
        ...
        for token, address in treasury_tokens.items():
            ...
            for countertoken in countertoken_csv.split(","):
                ...
                try:
                    ...
                    coingecko_price_gauge.labels(token, countertoken, address).set(
                        token_prices[treasury_tokens[token].lower()][countertoken]
                    )
                    usd_prices_by_token_address[address] = token_prices[
                        address.lower()
                    ]["usd"]
                    if token == "WETH":
                        coingecko_price_gauge.labels("ETH", countertoken, address).set(
                            token_prices[treasury_tokens[token].lower()][countertoken]
                        )
                except:
        ...
        for token in crvpools:
            ...
            virtual_price = (
                interface.CRVswap(crvpools[token]).get_virtual_price() / 1e18
            )
            usd_price = (
                virtual_price * usd_prices_by_token_address[treasury_tokens["WBTC"]]
            )
            crvtoken_gauge.labels(token, "pricePerShare", treasury_tokens["WBTC"]).set(
                virtual_price
            )
            crvtoken_gauge.labels(
                token, "usdPricePerShare", treasury_tokens["WBTC"]
            ).set(usd_price)
            usd_prices_by_token_address[treasury_tokens[token]] = usd_price

        #  Processing Bridged tokens
        custodians = {
            "multiswap": Web3.toChecksumAddress(
        ...
        for custodian, address in custodians.items():
            ...
            for token in native_tokens:
                ...
                xchain_bridge_gauge.labels("BSC", token, custodian, "balance").set(
                    ti.balanceOf(address) / (10 ** ti.decimals())
                )
                xchain_bridge_gauge.labels("BSC", token, custodian, "usdBalance").set(
                    ti.balanceOf(address)
                    / (10 ** ti.decimals())
        ...
        for metric in info.describe():
            ...
            wallets_gauge.labels(
                metric["walletName"], metric["walletAddress"], "ETH", "None", "balance"
            ).set(float(w3.fromWei(w3.eth.getBalance(wallet_web3_addr), "ether")))
            try:
                wallets_gauge.labels(
                    metric["walletName"],
                    metric["walletAddress"],
                    ...
                    float(w3.fromWei(w3.eth.getBalance(wallet_web3_addr), "ether"))
                ...
                wallets_gauge.labels(
                    ...
                    metric["tokenAddress"],
                    "usdBalance",
                ).set(metric["balance"] * usd_prices_by_token_address[tokenAddress])
            except:
        ...
        digg_sushi_price = (slpDiggWbtc.getReserves()[0] / 1e8) / (
            slpDiggWbtc.getReserves()[1] / 1e9
        )
        digg_uni_price = (uniDiggWbtc.getReserves()[0] / 1e8) / (
            uniDiggWbtc.getReserves()[1] / 1e9
        )

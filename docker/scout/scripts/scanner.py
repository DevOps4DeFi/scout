"""
source: https://web3py.readthedocs.io/en/stable/examples.html?highlight=newfilter#advanced-example-fetching-all-token-transfer-events
...
# but we need it to construct eth_getLoggers parameters
...
class EventScanner:
    ...
    because it cannot correctly throttle and decrease the `eth_getLoggers` block number range.
    """
    ...
    def __init__(
        ...
        :param filters: Filters passed to getLoggers
    ...
    def scan_chunk(self, start_block, end_block) -> Tuple[int, datetime.datetime, list]:
        ...
        for event_type in self.events:
            ...
            # Do `n` retries on `eth_getLoggers`,
    ...
    def scan(
        ...
        while current_block <= end_block:
            ...
            # Try to guess how many blocks to fetch over `eth_getLoggers` API next time
...
def _retry_web3_call(func, start_block, end_block, retries, delay) -> Tuple[int, list]:
    ...
    If our JSON-RPC server cannot serve all incoming `eth_getLoggers` in a single request,
...
def _fetch_events_for_all_contracts(
    ...
    if from_block is None:
        raise TypeError("Missing mandatory keyword argument to getLoggers: fromBlock")
    ...
    logger.debug(
        "Querying eth_getLoggers with the following parameters: %s", event_filter_params
    )
